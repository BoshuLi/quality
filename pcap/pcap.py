...
for f in files:
	...
	countRating = 0
	print f
	bk=xlrd.open_workbook('D:\\code\\pcap\\handle\\'+f)
	...
	for i in range(nrows):
		...
		if str(cell_value) not in doucument:
			countDev = countDev+1
	...
	sheet1.write(count, 1, countDev)
	sheet1.write(count, 2, countRating)
	sheet1.write(count, 3, float(countRating+1)/float(countDev+1))
